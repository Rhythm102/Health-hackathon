...
// MQTT Connection
client.on("connect", () => {
  console.log("✅ MQTT connected (Ambulance Messages)");
...
client.on("message", (topic, payload) => {
  try {
    ...
    if (msg.sender === "ambulance" && msg.isLocal) {
      return;
    }
...
function sendMessage() {
  ...
  addMessage("ambulance", msg.text, msg.timestamp);
...
window.sendQuickMessage = function(text) {
  ...
  client.publish(TOPIC, JSON.stringify(msg));
  addMessage("ambulance", text, msg.timestamp);
...
console.log(`🚑 Ambulance Messages page initialized in ${currentMode} mode`);